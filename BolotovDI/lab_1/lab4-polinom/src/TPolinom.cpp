...
void TPolinom::PrivPod()
{
	...
	while (A.monoms.IsEnded() == false)
	{
		res.monoms.Reset();
		if (res.monoms.IsEnded() == false)
		{
			while ((res.monoms.IsEnded() == false) && (res.monoms.GetLink()->data.abc != A.monoms.GetLink()->data.abc))
			{
				res.monoms.Next();
			}

			if (res.monoms.IsEnded() == true)
			{
				res.monoms.Insert(A.monoms.GetLink()->data);
			}

			if (res.monoms.GetLink()->data.abc == A.monoms.GetLink()->data.abc)
			{
				res.monoms.GetLink()->data.cf = res.monoms.GetLink()->data.cf + A.monoms.GetLink()->data.cf;
			}
		}
		else
		{
