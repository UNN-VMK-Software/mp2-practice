...
public:
	...
	friend TPolinom operator* (double d, TPolinom& _pol) { return _pol * d; };
	TPolinom& operator = (const TPolinom& _pol);
	...
	friend ostream& operator<< (ostream &out, const TPolinom& _pol)
	{
		...
		if ((copy_of_polinom.curmon.GetCurrent()->data.cf==1) && (copy_of_polinom.curmon.GetCurrent()->data.stmonom / 100 != 0) && (copy_of_polinom.curmon.GetCurrent()->data.stmonom / 10) % 10 != 0 && (copy_of_polinom.curmon.GetCurrent()->data.stmonom) % 10 != 0)
